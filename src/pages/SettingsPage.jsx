...
import { User, Palette, LogOut, Save, Loader2, ChevronRight, Moon, Sun, Shield, Camera, X } from "lucide-react";
import { toggleTheme } from "../features/themeSlice";
import { updateProfile, logoutUser, uploadProfileImage } from "../features/authSlice";
...
export default function SettingsPage() {
    ...
    const handleSaveAvatar = async () => {
        ...
        try {
            await dispatch(uploadProfileImage({ userId: currentUser.id, file: avatarFile })).unwrap();
            toast.success("Profile photo updated!");
            handleCancelAvatarChange();
        } catch (err) {
            toast.error(err || "Failed to upload photo. Make sure the 'avatars' storage bucket exists in Supabase.");
        } finally {
    ...
    // Determine which avatar to show in settings
    const displayedAvatar = avatarPreview || currentUser?.avatar_url;
    const initials = (name?.[0] || currentUser?.name?.[0] || "U").toUpperCase();
                                    ...
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div className="relative flex-shrink-0 group">
                                            <button
                                                ...
                                                className="relative size-20 rounded-full overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-900"
                                            ...
                                            >
                                                {displayedAvatar ? (
                                                    ...
                                                    <div className="size-20 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-blue-500/20">
                                                        {initials}
                                                    </div>
                                                )}
                                                {/* Hover overlay */}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-zinc-400 mb-3">{currentUser?.email}</p>
                                            ...
                                            {avatarPreview ? (
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <button
                                                        ...
                                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium shadow-sm shadow-blue-500/20 transition disabled:opacity-60 disabled:cursor-not-allowed"
                                                    >
                                                        {isUploadingAvatar ? (
                                                            <><Loader2 className="size-3 animate-spin" /> Uploading…</>
                                                        ) : (
                                                            <><Save className="size-3" /> Save photo</>
                                                        )}
                                                    </button>
                                                    <button
                                                        type="button"
                                                        ...
                                                        disabled={isUploadingAvatar}
                                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 transition disabled:opacity-60"
                                                    >
                                                        <X className="size-3" /> Cancel
                                                    </button>
                                                    <p className="text-xs text-gray-400 dark:text-zinc-500 w-full mt-1">Preview — click Save photo to apply</p>
                                                </div>
                                            ) : (
                                                <button
                                                    type="button"
                                                    onClick={() => fileInputRef.current?.click()}
                                                    className="text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium transition"
                                                >
